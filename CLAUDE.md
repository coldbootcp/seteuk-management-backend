...
- **인수인계**: `docs/HANDOFF.md` — 새로 합류한 사람이 이 문서 하나로 작업을 이어받기
  위한 글. 띄우는 법, 지켜야 할 경계, 지금 상태, 다음에 할 일, 시간을 많이 쓴 함정.
  **새 세션은 이 문서부터 읽을 것.**
- **미사용 백엔드 기능**: `docs/UNUSED_BACKEND_FEATURES.md` — 백엔드에 구현되어
  있지만 프론트엔드가 부르지 않는 엔드포인트 지도(진단 실행 트리거 부재, 후속 추천
  전체, 계획 탭, 활동 계보 화면 등). 화면에 무엇을 새로 붙일지 정할 때 먼저 볼 것.
...
- [x] **프론트엔드 통합 + 실사용 검증** — 소유자가 제공한 별도 프론트엔드
  ...
  Reviewer(`app/services/review.py`), CORS.
  **실제 생기부와 실제 키로 전 구간을 태우며 잡은 것들** — 모킹 테스트가 전부 통과하는
  상태에서 실 문서로만 드러났다: (1) 파서가 성적표 열 머리글("1학기 | 2학기")을 학기
  구분으로 오인해 1학년 세특 32건을 전부 2학기로 밀어 넣고 1학기를 비웠다 — 제 줄에 홀로
  서 있고 짝이 옆에 없는 라벨만 인정하도록 고쳤다(`anchors.semester_markers`).
  (2) 수상의 학년-학기를 파싱 시점에 채운다 — 표에 학년 열이 없지만 `참가대상`이 학년을,
  수상연월일이 학기를 알려 준다. 바깥 시계를 기준점으로 쓰면 몇 해 전 문서의 수상이 전부
  학년 범위 밖으로 떨어진다. (3) 진단이 학년 단위 활동 57건(157건 중)을 통째로 놓쳤다 —
  자율활동·진로활동은 학기가 없어 `semester IS NOT NULL` 조건에 걸렸고, 그 결과 활동이
  있는 학기를 "기록 없음"으로 단정했다. 이제 그 학년의 두 학기에 모두 근거로 넣되
  `year_activities`로 따로 담는다. (4) 파싱이 연결 타임아웃으로 활동 수십 건을 잃었다
  (156 → 97) — SDK 기본 connect 5초가 원인이었고, 일시적 실패만 재시도하도록 나눴다.
  (5) 출결이 어느 경로로도 반영되지 않았다 — 검토 화면에 출결 탭이 없어 선택에 실리지
  않는데 "지정되지 않은 영역은 반영에서 뺀다"는 규칙과 맞물렸다.
  **설계에서 뒤집은 것**: 마디의 `status`는 진척만 뜻하고, "학생이 지금 어느 학기에
  있는가"는 응답의 `is_current`가 답한다(저장하지 않고 매번 계산). 두 개념이 한 필드에
  섞여 있어 이번 학기 목표를 일찍 달성하면 화면이 다음 학기로 건너뛰었다.
  생기부는 **가장 최근 것 하나만** 보관하고(소유자 결정), 지난 업로드를 지울 때는 그
  업로드에서 온 기록을 새 업로드로 옮겨 붙인 뒤 지운다 — 그냥 지우면 `source_upload_id`가
  null이 되어 직접 입력한 행과 구분되지 않는다. `GET /seteuk/uploads/latest`를 더해
  클라이언트가 업로드 id를 잃어도 진행 상황을 되찾을 수 있게 했다.
...
- [x] **생기부 학년 대조 강화 + 캘린더 탭(시험/수행평가 기간)** — 생기부 업로드 시
  학년 대조 기준을 `users.current_grade`(자기 선언, 안 갱신하면 낡는 값)에서
  **입학 연도**로 바꿈(`_expected_period_from_freshman_year`) — 입학 연도는
  한 번 정해지면 거의 안 바뀌는 사실이라 더 믿을 수 있다. 문서의 최고
  (학년, 학기)와 입학 연도로 계산한 "지금쯤이어야 할 (학년, 학기)"를 비교해,
  **문서가 계산값을 넘으면**(같은 학년 안에서 학기만 넘는 경우 포함) 업로드
  자체를 반려하고(`status: failed`), **문서가 계산값보다 낮으면** 반영은 하되
  "더 최근 생기부를 올려달라"는 경고를 남긴다(`stale_school_record`). 같은
  학년 안에서 학기만 덜 채워진 경우(현재 학기가 막 시작해 학교가 아직 입력
  중)는 정상이므로 경고하지 않는다 — 학년 단위로만 "낡음"을 판단한다. 이
  과정에서 `GET /seteuk/uploads/{id}`가 `failure_reason`을 아예 안 내려주던
  실제 버그를 발견해 함께 고쳤다(프론트가 업로드 진행 중 폴링하는 게 바로 이
  엔드포인트라, 고치기 전까진 반려 이유가 화면에 전혀 안 보였을 것). **실제
  tmp.pdf + 실제 DeepSeek 파서로 라이브 검증**: 입학 연도를 일부러 낮게
  선언하고 올렸더니 "3학년 2학기 기록이 있는데 2025학년도 입학 기준으로는
  아직 2학년 2학기여야 합니다" 형태로 정확히 반려됨을 확인.
  같은 세션에서 **캘린더 탭**(시험·수행평가 기간 기록용)도 추가함 — 6개 탭과
  같은 `build_record_router` 팩토리를 그대로 재사용하되(월 격자 + 일정
  목록·추가/삭제, 색으로 구분된 점으로 날짜 칸에 기간 표시), 생기부에서
  파싱되는 게 아니라 학생이 직접 적는 "앞으로 있을 일"이라 `source_upload_id`
  는 없음. 만들면서 **또 다른 실제 버그를 발견**: `model_validator`가
  `raise ValueError(...)`로 만든 검증 오류(예: 종료일이 시작일보다 빠름)가
  오면 `app/main.py`의 공통 `RequestValidationError` 핸들러 자체가
  500으로 죽었다 — `exc.errors()`의 `ctx.error`에 원본 예외 객체가 그대로
  담겨 있어 `JSONResponse`의 기본 `json.dumps`가 직렬화하지 못했기 때문.
...
- **카카오 소셜 로그인은 미검증이다.** 백엔드·프론트엔드 코드는 붙였지만 실제 카카오 앱
  키가 없다. 프론트엔드 `.env.local`에 `NEXT_PUBLIC_KAKAO_JS_KEY`를 넣으면 로그인
  화면에 버튼이 나타난다(키가 없으면 버튼 자체를 그리지 않는다).
