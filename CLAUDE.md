...
- [x] 진단 4단계 — 챗봇 말투 리포트(`narrative_report`) — 진단 완료 후 사용자에게 보여줄 것은 구조화된 필드 나열이 아니라 하나로 이어지는 글이어야 한다는 요구를 받아, 3단계(종합) 결과만 입력받는 4번째 LLM 단계를 추가함(원본 데이터 재조회 없음 — 3단계와 같은 "이전 단계 결과만 입력" 원칙 유지). 진단당 1회만 호출되어 `diagnoses.narrative_report`에 저장되므로 조회할 때마다 다시 LLM을 부르지 않고 같은 문구를 돌려줌(비용·일관성 모두 확보). 프롬프트는 강점/약점/진로스토리를 따로 나열하지 말고 하나의 이야기로 엮으라는 규칙과, 챗봇 프롬프트와 동일한 존댓말 강제 규칙을 명시함. **실제 DeepSeek로 검증**하다가 두 가지를 발견·수정함: (1) 실제 생기부로 만든 사용자에 대해 전체 진단(1~4단계 동시 실행)을 다시 돌렸더니 DeepSeek 계정의 동시성 한도(잔액 기반, 10)에 걸려 3회 재시도 후 `failed`로 정확히 처리됨(설계대로 — 코드 결함 아님, 계정 잔액/동시 요청 제약). 이 때문에 4단계만 독립적으로(동시성 부하 없이) 검증하기 위해 실제로 완료됐던 이전 진단의 3단계 결과를 그대로 입력 삼아 `_write_narrative_report`를 단발 호출함. (2) 그렇게 4회 반복 호출한 결과 매번 "5~10개 문단"이라는 프롬프트 지시를 어기고 **문단 구분 없이 한 덩어리(700~1200자)로만** 썼음 — JSON 문자열 안에 줄바꿈을 넣는 것을 회피하는 것으로 보임. 프롬프트 규칙에 "문단 사이는 JSON 이스케이프 시퀀스 `\n\n`으로 띄워라"를 명시적으로 추가한 뒤 재검증하니 4회 모두 4~6개 문단으로 정상 분리되고 각 문단이 순서대로 진로 스토리→강점→약점→갭 분석/방향을 담음(반말 없음, 마크다운 헤더·글머리 기호 없음도 확인)
