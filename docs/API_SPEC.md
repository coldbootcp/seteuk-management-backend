...
```
검토를 마친 결과 중 고른 것만 반영한다. 각 항목은 결과 배열의 index 목록이다.

**대체 범위는 이번에 지정한 영역까지다.**

- 영역을 **하나도 지정하지 않으면** 전체 반영이고, 이전 반영분을 통째로 대체한다.
- **일부 영역만 지정하면** 그 영역만 손댄다. 나머지 영역의 이전 반영분은 그대로
  남는다 — 검토 화면이 [상장]·[활동]처럼 카테고리별로 나눠 반영하기 때문에, 매번
  전부 비우면 상장을 반영하는 순간 앞서 반영한 성적이 사라진다.
- 지정한 영역에 **빈 배열**을 주면 그 영역을 비운다는 뜻이다.

응답은 영역별로 실제 몇 건이 들어갔는지 돌려준다. 직접 입력한 행(`source_upload_id`가
비어 있는 것)은 어느 경우에도 건드리지 않는다.
...
온보딩 저장 전에도 `freshman_academic_year`를 함께 보낸다. 이 요청은 **현재 성적 수준,
잘하는·어려운 과목, 과목별 강점·약점**을 묻지 않는다. 이는 성적·수강 과목·학생부
기록으로 판단할 진단 영역이기 때문이다. 모델이 5등급제 대상자에게 6~9등급 선택지처럼
등급 체계 밖의 보기를 만들면 서버가 해당 질문을 버린다.
