"""서울 주요대학 학과의 교육목표·교육과정·진로 정보를 적재한다."""
...
from app.services.admission_catalog_service import sync_program_profile_sections
...
async def _sync_with_retry(
    ...
    for attempt in range(1, MAX_RETRIES + 1):
        try:
            return await sync_program_profile_sections(db, reference, university)
...
async def main() -> None:
    async with AsyncSessionLocal() as db:
        ...
        profiles_with_sections = 0
        for index, (reference, university) in enumerate(rows, 1):
            section_count = await _sync_with_retry(db, reference, university)
            profiles_with_sections += int(section_count > 0)
            await db.commit()
            print(
                f"[{index}/{len(rows)}] {university.name} / {reference.name}: "
                f"{section_count} sections (profiles {profiles_with_sections})"
            )
